...
def upgrade() -> None:
    ...
    op.create_table(
        "devices",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
        sa.Column("device_code", sa.String(64), unique=True, nullable=False, index=True),
        sa.Column("name", sa.String(255), nullable=False),
        sa.Column("device_type", sa.String(64), server_default="manhole_cover"),
        sa.Column("status", sa.Enum("online", "offline", "fault", "maintenance", name="device_status"), server_default="offline"),
    ...
    op.create_table(
        ...
        sa.Column("protocol", sa.Enum("rtsp", "hls", "webrtc", "local", name="stream_protocol"), server_default="rtsp"),
    ...
    op.create_table(
        ...
        sa.Column("alert_type", sa.Enum(
            ...
            "device_offline", "system_error", name="alert_type",
        ), nullable=False),
        sa.Column("level", sa.Enum("critical", "warning", "info", name="alert_level"), server_default="info"),
    ...
    op.create_table(
        ...
        sa.Column("status", sa.Enum("loading", "active", "unloading", "error", name="model_status"), server_default="loading"),
    ...
    op.create_table(
        "inference_results",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
        sa.Column("camera_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("camera_streams.id", ondelete="CASCADE")),
