...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    mockAPI
      .getStatus()
      .then((res) => setMockRunning(res.data.running))
      .catch(() => {})
    mockAPI
      .getConfig()
      .then((res) => setMockConfig(res.data))
      .catch(() => {})
  }, [])
            ...
            children: (
              <div className="space-y-4">
                <Card
                  title="当前模型状态"
                  size="small"
                  className="bg-cyber-dark border-cyber-border"
                >
                  <Descriptions
                    column={2}
                    size="small"
                ...
                <Card title="模型热切换" size="small" className="bg-cyber-dark border-cyber-border">
                  <Space>
                    <Select
                      ...
                      className="w-40"
                      options={Object.keys(modelStatus?.registry || {}).map((v) => ({
                        value: v,
                        label: v,
                      }))}
                    />
                    ...
                    <Button icon={<ReloadOutlined />} onClick={fetchModelStatus}>
                      刷新
                    </Button>
                  </Space>
                </Card>
                  ...
                  <Card title="版本仓库" size="small" className="bg-cyber-dark border-cyber-border">
                    <div className="space-y-2">
                      {Object.entries(modelStatus.registry).map(([v, info]) => (
                        <div
                          key={v}
                          className="flex items-center justify-between p-2 bg-cyber-black/50 rounded"
                        >
                  ...
                  >
                    <Descriptions.Item label="PostgreSQL">
                      {statusLoading ? (
                        ...
                        <Tag
                          color={systemStatus?.postgresql?.status === 'connected' ? 'green' : 'red'}
                        >
                    ...
                    <Descriptions.Item label="InfluxDB">
                      {statusLoading ? (
                        ...
                        <Tag
                          color={
                            systemStatus?.influxdb?.status === 'connected'
                              ? 'green'
                              : systemStatus?.influxdb?.status === 'not_configured'
                                ? 'default'
                                : 'red'
                          }
                        >
                      ...
                      {statusLoading ? (
                        ...
                        <Tag color={systemStatus?.model?.status === 'ready' ? 'green' : 'orange'}>
                          {systemStatus?.model?.status === 'ready'
                            ? `就绪 (${systemStatus?.model?.active_version})`
                            : '未就绪'}
                        </Tag>
                      )}
                    </Descriptions.Item>
