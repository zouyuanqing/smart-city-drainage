...
function DeviceCard({ device, reading }: { device: Device; reading?: SensorReading }) {
  const statusColor = device.status === 'online' ? '#00FF88' :
    device.status === 'fault' ? '#FF3366' : '#667788';
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-mono text-neon-blue">
              {reading.water_level_mm.toFixed(0)} <span className="text-[10px] text-text-muted">mm</span>
            </span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-text-muted">流量</span>
            <span className="font-mono text-neon-cyan">
              {reading.flow_rate_m3h.toFixed(1)} <span className="text-[10px] text-text-muted">m³/h</span>
            </span>
          </div>
          <div className="flex justify-between text-xs">
...
function StatsBar() {
  ...
  const onlineCount = Array.from(sensorReadings.values()).filter(
    r => r.battery_level > 0
  ).length;
  ...
  return (
    <div className="grid grid-cols-3 lg:grid-cols-5 gap-3">
      {[
        { label: '在线设备', value: onlineCount, unit: '/8', color: '#00FF88', icon: <ThunderboltOutlined /> },
        { label: '总告警', value: totalAlerts, unit: '条', color: '#FF8C00', icon: <CloseCircleOutlined /> },
        { label: '紧急告警', value: criticalAlerts, unit: '条', color: '#FF3366', icon: <CloseCircleOutlined /> },
        { label: 'AI 推理', value: 'v1', unit: 'active', color: '#00D4FF', icon: <SettingOutlined /> },
        { label: '网络延迟', value: '<10', unit: 'ms', color: '#A855F7', icon: <WifiOutlined /> },
      ].map((s, i) => (
        <div key={i} className="bg-cyber-dark border border-cyber-border rounded-lg p-3 text-center">
          <div className="text-text-muted text-[10px] font-mono uppercase tracking-wider mb-1">{s.label}</div>
...
export function Dashboard() {
  const {
    ...
    alerts, addAlert, acknowledgeAlert,
    devices, setDevices,
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      <div className="flex-1 grid grid-cols-12 gap-3 min-h-0">
        ...
        <div className="col-span-12 lg:col-span-2 flex flex-col gap-3">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
            {(filteredDevices.length > 0 || devices.length > 0 ? (filteredDevices.length > 0 ? filteredDevices : devices) : devices).map(device => (
              <div
                key={device.id}
                onClick={() => {
              ...
              >
                <DeviceCard
                  device={device}
                  reading={sensorReadings.get(device.id)}
                />
              </div>
            ))}
          </div>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-3">
          ...
          <div className={alertsCollapsed ? '' : 'flex-1 min-h-0'}>
            <div className="panel-title cursor-pointer select-none"
                 onClick={() => setAlertsCollapsed(!alertsCollapsed)}>
              <CloseCircleOutlined /> 实时告警
              <Badge count={alerts.filter(a => !a.is_acknowledged).length}
      ...
      <Drawer
        title={
          <span className="font-display text-neon-blue tracking-wider">
            📹 视频监控详情
          </span>
        }
        placement="right"
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <div className="video-container aspect-video">
            ...
            <div className="absolute top-2 left-2 z-10">
              <Tag color="cyan" className="font-mono text-[10px]">RTSP → HLS</Tag>
            </div>
          </div>
          <div className="video-container aspect-video">
            <VideoPlayer streamType="local" />
            <div className="absolute top-2 left-2 z-10">
              <Tag color="purple" className="font-mono text-[10px]">本地摄像头</Tag>
            </div>
          </div>
        </div>
