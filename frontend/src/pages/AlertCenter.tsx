...
import { Tabs, Select, Button, message, Badge, Descriptions, Tag } from 'antd';
import {
  CheckCircleOutlined,
  CloseCircleOutlined,
  ReloadOutlined,
...
export function AlertCenter() {
  const { alerts, addAlert, acknowledgeAlert, resolveAlert, selectedAlertId, setSelectedAlert } = useAppStore();
    ...
    <div className="h-full flex flex-col gap-3">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="bg-cyber-dark border border-cyber-border rounded p-3 text-center">
            <div className="data-value text-lg" style={{ color: s.color }}>{s.value}</div>
          ...
          {selectedAlert ? (
            <div>
              ...
              <Descriptions column={2} size="small" bordered
                styles={{ label: { color: '#8899AA', fontSize: 11 }, content: { color: '#E8EDF2', fontSize: 11 } }}>
                <Descriptions.Item label="告警类型">{selectedAlert.alert_type}</Descriptions.Item>
                <Descriptions.Item label="级别">
                  <span style={{ color: selectedAlert.level === 'critical' ? '#FF3366' : '#FF8C00' }}>
                    {selectedAlert.level}
                  </span>
                </Descriptions.Item>
                <Descriptions.Item label="设备">{selectedAlert.device_name || '--'}</Descriptions.Item>
                <Descriptions.Item label="状态">
                  <Badge
                    status={selectedAlert.is_resolved ? 'success' : selectedAlert.is_acknowledged ? 'processing' : 'error'}
                    text={selectedAlert.is_resolved ? '已解决' : selectedAlert.is_acknowledged ? '已确认' : '未处理'}
                  />
                </Descriptions.Item>
                <Descriptions.Item label="置信度">
                  {selectedAlert.detection_confidence ? `${(selectedAlert.detection_confidence * 100).toFixed(1)}%` : '--'}
                </Descriptions.Item>
