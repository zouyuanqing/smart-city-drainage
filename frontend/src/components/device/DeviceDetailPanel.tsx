...
export function DeviceDetailPanel({ open, onClose }: DeviceDetailPanelProps) {
  ...
  const statusColor =
    device?.status === 'online' ? 'green' : device?.status === 'fault' ? 'orange' : 'default'
        ...
        {reading ? (
          <div className="grid grid-cols-3 gap-3">
            {[
              {
                label: '液位',
                value: `${reading.water_level_mm.toFixed(0)}`,
                unit: 'mm',
                color: '#00D4FF',
              },
              {
                label: '流量',
                value: `${reading.flow_rate_m3h.toFixed(1)}`,
                unit: 'm³/h',
                color: '#00F5FF',
              },
              {
                label: '温度',
                value: `${reading.temperature_c.toFixed(1)}`,
                unit: '°C',
                color: '#FF8C00',
              },
              {
                label: '湿度',
                value: `${(reading.humidity_pct || 0).toFixed(0)}`,
                unit: '%',
                color: '#A855F7',
              },
              {
                ...
                unit: '%',
                color: reading.battery_level > 30 ? '#00FF88' : '#FF3366',
              },
              {
                label: '信号',
                value: `${reading.signal_strength}`,
                unit: '%',
                color: reading.signal_strength > 50 ? '#00FF88' : '#FF8C00',
              },
            ].map((m) => (
              <div
                key={m.label}
        ...
        {deviceAlerts.length > 0 ? (
          <div className="space-y-2">
            {deviceAlerts.slice(0, 10).map((a) => (
              <div
                key={a.id}
                className="flex items-center justify-between p-2 bg-cyber-dark border border-cyber-border rounded"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-text-muted text-[10px] font-mono">
                    ...
                    <Tag
                      color={
                        a.level === 'critical' ? 'red' : a.level === 'warning' ? 'orange' : 'blue'
                      }
