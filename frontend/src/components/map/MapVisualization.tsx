...
const DeviceMarker = memo(function DeviceMarker({
  ...
  const color = hasAlert ? COLORS.alert
    : device.status === 'online' ? COLORS.online
    : device.status === 'fault' ? COLORS.fault
    : COLORS.offline;
...
// ---- 告警覆盖层 ----
function AlertOverlay({ alerts, onAlertClick }: {
  alerts: Alert[];
        ...
        {alerts.filter(a => !a.is_acknowledged).slice(0, 5).map(alert => (
          <motion.div
            ...
            className={`p-3 rounded-lg cursor-pointer backdrop-blur-md border pointer-events-auto
              ${alert.level === 'critical' ? 'bg-neon-red/10 border-neon-red/50' :
                alert.level === 'warning' ? 'bg-neon-orange/10 border-neon-orange/50' :
...
// ---- 地图控制 ----
function MapController({ selectedDeviceId, devices }: {
...
function MapLegend() {
  ...
  return (
    <div className="absolute bottom-3 left-3 z-[1000] bg-cyber-dark/90 backdrop-blur-sm
...
export function MapVisualization({
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* 左上角 HUD 信息 */}
      <div className="absolute top-3 left-3 z-[1000] bg-cyber-dark/80 backdrop-blur-sm
