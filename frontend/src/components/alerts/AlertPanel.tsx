...
function AlertItem({ alert, onAcknowledge, onClick }: {
  alert: Alert;
  onAcknowledge: (id: string) => void;
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {alert.device_name && (
            <div className="text-text-muted text-[10px] font-mono truncate">{alert.device_name}</div>
          )}
  ...
  if (sortedAlerts.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <Empty description={<span className="text-text-muted">暂无告警</span>}
